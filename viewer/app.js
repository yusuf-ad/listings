...
function renderOverview(listing) {
  ...
  panel.querySelectorAll('[data-field]').forEach(input => {
    input.addEventListener('input', (e) => {
      setNestedValue(listing, e.target.dataset.field, e.target.type === 'number' ? (e.target.value === '' ? null : Number(e.target.value)) : e.target.value);
      markChanged();
    });
  });
}
...
function renderAmenities(listing) {
  ...
  document.getElementById('btnAddAmenity').addEventListener('click', addAmenity);
  document.getElementById('newAmenityInput').addEventListener('keydown', (e) => {
